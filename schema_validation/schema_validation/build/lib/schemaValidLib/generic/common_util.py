...
class Common_Utils(Spark):
  ...
  def getEventPath(self, eventName):
    if eventName in self.constant.event:
      eventGun = self.constant.event[eventName]
      return eventGun
    else:
      raise eventError(eventName)
  ...
  def getOrgPath(self, orgName):
    if orgName in self.constant.originator:
      orgGun = self.constant.originator[orgName]
      return orgGun
    else:
      raise originatorError(orgName)
  ...
  def filtered_forPlatformFamilies(self, dataFrame):
    ...
    if isinstance(platform_families, str):
      platform_families = list(platform_families.split(","))
    if platform_families == ['']:
      platform_families = self.constant.platformFamilies
      self.is_allPlatormFamilies = True
    print(f'dataframe is filtered for platform families: {platform_families}')
  ...
  def loadDf(self):
    ...
    try:
      ...
      print(f'originator gun: {self.getOrgPath(originator)}')
      print(f'event gun: {self.getEventPath(event)}')
