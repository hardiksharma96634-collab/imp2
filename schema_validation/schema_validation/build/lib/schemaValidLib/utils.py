...
class Utils(Common_Utils):
  ...
  def get_requiredProductFamilyNames(self): 
    ...
    productFamilyNames = [productFamily for productFamily in productFamilyNames.split(",")]
  ...
  def getSchemaValidationProcessed(self,df,otherColumns=[],filterExp=None,joinType=None):
    ...
    self.filter_productFamilies = self.get_requiredProductFamilyNames()
      ...
      if(generate_payloads.upper()=="YES"):
        for payload_type,payload in allTypePayloads.items():
          if self.is_validPayload(payload):
            print(f"Here is the {payload_type} for {self.schemaError}:")
